...
const HotelProfile = () => {
  ...
  return (
    <div className="hotel-form">
      <div className="container mx-auto py-5 mt-5">
        <div className="flex justify-center">
          {hotelDetails && hotelDetails.map((hotel, index) => (
            <div className="w-full md:w-3/4 lg:w-2/3 xl:w-1/2" key={index}>
              <div>
                <div className="bg-white shadow-lg p-4 rounded-lg">
                  <div className="rounded-t bg-green-100 text-white text-center py-8">
                    <div className="mb-4">
                      <img
                        src={`http://localhost:5000/images/${hotel.images[0]}`}
                        alt={`Profile Image ${index}`}
                        className="w-32 h-32 object-cover mx-auto rounded-full"
                      />
                    </div>
                    <button onClick={handleEditClick} className="btn btn-outline mt-2">Edit my home</button>
    ...
    <div className="w-full md:w-1/2 lg:w-1/3 xl:w-1/4 p-4 bg-white rounded-lg">
      <p className="italic text-black">{hotel.hotelName}</p>
    </div>
    <div className="w-full md:w-1/2 lg:w-1/3 xl:w-1/4 p-4 bg-white rounded-lg pl-4 ">
      <p className="italic">{hotel.roomType}</p>
    </div>
    <div className="w-full md:w-1/2 lg:w-1/3 xl:w-1/4 p-4 bg-white rounded-lg">
      <p className="italic">{hotel.roomPrice}</p>
    </div>
    <div className="w-full md:w-1/2 lg:w-1/3 xl:w-1/4 p-4 bg-white rounded-lg">
      <p className="italic">{hotel.packageLocation}</p>
    </div>
    <div className="w-full md:w-1/2 lg:w-1/3 xl:w-1/4 p-4 bg-white rounded-lg">
      <p className="italic">{hotel.address}</p>
    </div>
    <div className="w-full md:w-1/2 lg-w-1/3 xl:w-1/4 p-4 bg-white rounded-lg">
      <p className="italic">{hotel.contactNumber}</p>
    </div>
    <div className="w-full md-w-1/2 lg:w-1/3 xl:w-1/4 p-4 bg-white rounded-lg">
      <p className="italic">{hotel.speciality}</p>
    </div>
    <div className="w-sm md-w-1/2 lg:w-1/3 xl:w-1/4 p-4 bg-white rounded-lg">
      <p className="italic">{hotel.services}</p>
    </div>
                        ...
                        <div className="flex" key={index}>
                          {hotel.images.map((image, imgIndex) => (
                            <img
                              ...
                              className="w-32 h-32 object-cover mx-4"
                              key={imgIndex}
                            />
                          ))}
                        </div>
          ...
          <div className="w-1/4 mx-4 mt-4">
            <div className="bg-white shadow-lg p-4 rounded-lg">
              <div className="text-center">
                ...
                />
                <button onClick={ffff} className="btn btn-outline mt-2">Change Photo</button>
              </div>
              {isEditing ? (
                <div className="p-4">
                  <input
                ...
                <div className="p-4">
                  <p className="font-semibold mb-2">{userDetails && userDetails[0] ? userDetails[0].name : ''}</p>
                  <p className="text-sm text-gray-500">{userDetails && userDetails[0] ? userDetails[0].email : ''}</p>
                </div>
              )}
            </div>
          </div>
